...
- El usuario ve los planes publicados por la comunidad de su ciudad en dos vistas alternativas: lista o mapa.
- El toggle lista ↔ mapa está disponible en la parte superior de la pantalla Explorar.
- En vista lista: tarjetas con foto, nombre, zona, fecha y cupos.
- En vista mapa: pins sobre el mapa mostrando la zona aproximada de cada plan (centro del barrio, no la dirección exacta). Al tocar un pin aparece una card resumen del plan.
- En ambas vistas puede filtrar por zona, categoría, día de la semana, tamaño del grupo y si es gratuito o pago.
- Los filtros aplicados en una vista se mantienen al cambiar a la otra.
...
- Mapa de calor o clustering avanzado de planes
...
**Vista mapa:**
- Mapa de MapKit (iOS nativo) centrado en la ciudad del usuario.
- Un pin por plan en la zona aproximada del evento (geocodificación del campo "zona / barrio" al guardar el plan).
- Al tocar un pin: aparece una card flotante mini con foto, nombre, fecha y cupos disponibles. Tap en la card navega al detalle del plan.
- Si hay múltiples planes en la misma zona: los pins se agrupan con un número. Tap en el cluster hace zoom hasta separar los pins.
- Los filtros activos en vista lista se aplican también al mapa.
- **Regla de privacidad:** el pin siempre cae en el centroide del barrio o zona ingresada, nunca en la dirección exacta. La dirección exacta se revela únicamente a los participantes confirmados dentro del detalle del plan.
...
- **Mapa interactivo:** MapKit (iOS nativo vía react-native-maps). Sin costo adicional de API para renderizado de mapas en iOS. Requiere geocodificación de zona/barrio a coordenadas lat/lng al crear un plan — usar Apple Geocoder (CLGeocoder, sin costo) o Google Maps Geocoding API (costo por request). Las coordenadas aproximadas (centroide de zona) se almacenan en la tabla `plans` (`location_lat`, `location_lng`). La dirección exacta se almacena en un campo separado (`location_exact`) visible solo para confirmados.
...
- Mapa de calor o clustering avanzado de planes
- Navegación turn-by-turn o integración con Apple Maps / Google Maps
